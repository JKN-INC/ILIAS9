...
class ilLPStatusManualByTutor extends ilLPStatus
{
    ...
    public function determineStatus(
        ...
        switch ($this->ilObjDataCache->lookupType($a_obj_id)) {
            case "crs":
            case "grp":
                // completed? also check failed flag from obj_members to support manual fail status
                $set = $this->db->query(
                    "SELECT ut.usr_id, passed, failed FROM ut_lp_marks ut " .
